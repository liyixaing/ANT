...
//            Log.e("时间戳times", times.toString())
//            Log.e("时间戳", nowtime)
            val sp = getSharedPreferences("USER", Context.MODE_PRIVATE)
//            Log.e("用户token：", sp.getString("user_token", ""))
            val strings = sp.getString("user_token", "")
//            Log.e("StringToken", strings)
            val token = sp.getString("user_token", "")
            val retrofit = Retrofit_manager.getInstance().userlogin
//            Log.e("啊实打实大所", MyApplication.keyAddressBeans.name)
//            Log.e("私钥", SharedPreferencesUtil.getData("userPrivatelyKey", "").toString())
            ...
            val transfer = retrofit.create(MoneyService::class.java).conversion(
                    ...
                    SignatureUtil.signtureByPrivateKey(token + nowtime),
                    MessageSignUtils.Sign(Credentials.create(keyPair), Numeric.toHexString(VerifyUtlis.toHash(nowtime + token))
                            .substring(2, Numeric.toHexString(VerifyUtlis.toHash(nowtime + token)).length)))
