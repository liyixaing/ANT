    ...
    override fun init() {
        ...
        if (intent.getStringExtra("leadKeyUrl") == "11") {
            val sp = this.getSharedPreferences("USER", Context.MODE_PRIVATE)
             leadKeyWebView.loadUrl(Retrofit_manager.WEBURL+"/wallet/src/importWallet.html?user_id=" + sp.getString("user_id", "") + "&user_token=" + sp.getString("user_token", "") + "&language=" + sp.getString("language", "") + "&loginType=android")
