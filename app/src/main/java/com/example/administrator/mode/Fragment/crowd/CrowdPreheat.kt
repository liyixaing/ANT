...
import kotlinx.android.synthetic.main.crowd_preheat.*
...
class CrowdPreheat : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        try {
            crowdPreheat.setONLoadMoreListener {
                                num+=10
                                loadfriend()
                                crowdPreheat.setLoadCompleted()
    ...
    private fun loadfriend() {
        try {
            ...
            val register = retrofit.create(MoneyService::class.java).getZhongChouList(sp.getString("user_id", ""), nowtime, sp.getString("user_token", ""), SignatureUtil.signtureByPrivateKey(sp.getString("user_token", "") + nowtime), PreferencesUtil.get("language", ""), "0", "0", "1", "2", num.toString(), "10")
                        ...
                        try {
                            ...
                            baseadaptr = object : FenAdater<getZhongChouListBean.DataBean>(activity, listInput, R.layout.crowdfunding_layout) {
                                override fun convert(holder: ViewHolder?, item: getZhongChouListBean.DataBean?) {
                                    ...
                                    holder.setText(R.id.timeRemainingTextView, "1:" + item.rate)
                                    ...
                                    if (item.status == 0) {
                                        holder.setText(R.id.crowdType, getString(R.string.haveNotStarted))
                                        holder.setText(R.id.timeRemainingTextView, getString(R.string.haveNotStarted))
                                    }
                                }
                            }
                            crowdPreheat.adapter = baseadaptr
                        } catch (e: Exception) {
