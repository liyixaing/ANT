...
public class SlideRightViewDragHelper extends LinearLayout {
    ...
    public SlideRightViewDragHelper(Context context, AttributeSet attrs) {
        super(context, attrs);        //新建viewDragHelper ,viewGroup, 灵敏度，回调(子view的移动)
            viewDragHelper = ViewDragHelper.create(this, 0.8f, new ViewDragHelper.Callback() {
                @Override
                ...
                public void onViewReleased(View releasedChild, float xvel, float yvel) {
                    if (oldX > 500) {
                        ...
                        invalidate();
                        //必须刷新,因为其内部使用的是mScroller.startScroll，所以别忘了需要invalidate()以及结合computeScroll方法一起。
                        if (onReleasedListener != null) onReleasedListener.onReleased();
